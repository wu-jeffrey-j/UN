...
def upload_mp3_to_gcs(bucket, mp3_file, relative_path):
    ...
    try:
        ...
        if blob_exists(bucket, blob_name):
            logger.info(f"⏭Skipped (already exists): {mp3_file} -> gs://{GCS_BUCKET_NAME}/{blob_name}")
            return True
        ...
        logger.info(f"Uploaded: {mp3_file} -> gs://{GCS_BUCKET_NAME}/{blob_name}")
        return True
...
def extract_and_upload_folders(source_dir, max_workers=8, delete_source=False):
    ...
    logger.info(f"Successfully uploaded: {total_uploaded} files")
